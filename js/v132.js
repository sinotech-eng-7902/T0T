...
function inputConfirmDialog(opts){opts=opts||{};var title=opts.title||'確認操作',requiredText=opts.requiredText||'',message=(opts.message||'')+(requiredText?'\n\n請在下方輸入完整文字：「'+requiredText+'」':''),label=requiredText?'請輸入完整確認文字':'請輸入確認文字';return openDialog({title:title,message:message,danger:!!opts.danger,inputLabel:label,requiredText:requiredText,confirmText:opts.danger?'確認刪除':'確定'}).then(function(result){return result.ok?result.value:null})}
...
function filteredResultResponses(f){var list=responses.slice(),search=String(resultDetailState.search||'').trim().toLowerCase(),dep=resultDetailState.department||'';if(search)list=list.filter(function(r){return resultMemberKeyword(r).includes(search)});if(dep)list=list.filter(function(r){return (r.departmentName||r.respondentDepartment||'')===dep});if(resultDetailState.sort==='oldest')list.sort(function(a,b){return resultTimeValue(a)-resultTimeValue(b)});else if(resultDetailState.sort==='newest')list.sort(function(a,b){return resultTimeValue(b)-resultTimeValue(a)});else list.sort(function(a,b){return String(a.departmentName||a.respondentDepartment||'').localeCompare(String(b.departmentName||b.respondentDepartment||''),'zh-Hant')||String(a.employeeNo||a.respondentEmployeeId||'').localeCompare(String(b.employeeNo||b.respondentEmployeeId||''),'zh-Hant',{numeric:true})||String(a.memberName||a.respondentName||'').localeCompare(String(b.memberName||b.respondentName||''),'zh-Hant')});return list}
function responseDetailRow(f,qs,r,manage){var id=attr(r.id),method='<b>'+esc(submissionMethodLabel(r))+'</b>'+(r.submissionMethod==='assisted'?'<br><small>由 '+esc(submitterLabel(r))+' 協助填寫</small>':''),actions=manage?actionGroup([actionButton('編輯',"openResponseEditor('"+id+"')"),actionButton('刪除',"deleteResponse('"+id+"')",'danger')]):roleBadgeHtml('唯讀',false);return '<tr>'+(f.identityMode==='member'?'<td>'+esc(r.departmentName||r.respondentDepartment||'')+'</td><td>'+esc(r.memberName||r.respondentName||'')+'</td><td>'+esc(r.employeeNo||r.respondentEmployeeId||'')+'</td>':'')+qs.map(function(q){return '<td>'+esc(answerText(q,r))+'</td>'}).join('')+'<td>'+esc(r.submittedAtText||formatAnyDate(r.submittedAt)||'')+'</td><td>'+method+'</td><td>'+actions+'</td></tr>'}
function renderResults(){var f=activeForm(),progress=f&&f.identityMode==='member'?completionData(f):null;$('resultCaption').textContent=f?f.title+'：共 '+responses.length+' 份回覆'+(progress?'；應填 '+progress.expected.length+' 人，未填 '+progress.missing.length+' 人':''):'請先選擇問卷。';if(!f){$('resultAnalysis').innerHTML='';resultsTable.innerHTML='';var p=$('missingResponsesPanel');if(p)p.style.display='none';return}$('resultAnalysis').innerHTML=renderAnalysis(f);renderMissingMembers(f);ensureResultDetailTools(f);var qs=normalizeQuestions(f.questions||[]).filter(function(q){return q.type!=='image'}),identityHeaders=f.identityMode==='member'?['部門','姓名','員工編號']:[],manage=canManageForm(f.id),list=filteredResultResponses(f);resultsTable.innerHTML=table(identityHeaders.concat(qs.map(function(q){return q.title}),['送出時間','填寫方式','操作']),list.map(function(r){return responseDetailRow(f,qs,r,manage)}),emptyState('查無填寫明細','請調整搜尋或篩選條件。'))}
function exportFilteredResults(){var f=activeForm();if(!f)return notify('請先選擇問卷');var qs=normalizeQuestions(f.questions||[]).filter(function(q){return q.type!=='image'}),list=filteredResultResponses(f),rows=list.map(function(r){var row={};if(f.identityMode==='member'){row['部門']=r.departmentName||r.respondentDepartment||'';row['姓名']=r.memberName||r.respondentName||'';row['員工編號']=r.employeeNo||r.respondentEmployeeId||''}qs.forEach(function(q){row[q.title]=answerText(q,r)});row['送出時間']=r.submittedAtText||formatAnyDate(r.submittedAt)||'';row['填寫方式']=submissionMethodLabel(r);row['協助填寫者']=r.submissionMethod==='assisted'?submitterLabel(r):'';return row});setPageLoading(true,'正在匯出填寫明細');try{var wb=XLSX.utils.book_new(),sheet=rows.length?XLSX.utils.json_to_sheet(rows):XLSX.utils.aoa_to_sheet([['查無填寫明細']]);XLSX.utils.book_append_sheet(wb,sheet,'填寫明細');XLSX.writeFile(wb,(f.title||'問卷')+'_填寫明細.xlsx');toast('填寫明細已匯出','success')}catch(e){console.error(e);notify('填寫明細匯出失敗','error')}finally{setPageLoading(false)}}
...
function ensureResultDetailTools(f){var head=document.querySelector('#resultsPanel .resultDetailHead');if(!head||!f)return;var existing=$('resultDetailTools');if(existing&&existing.dataset.formId===f.id)return;if(existing)existing.remove();var deps=resultDepartmentsForForm(f),html='<div id="resultDetailTools" class="resultDetailTools" data-form-id="'+attr(f.id)+'"><label>搜尋<input id="resultSearchInput" type="search" placeholder="姓名、部門、員編" value="'+attr(resultDetailState.search||'')+'" oninput="resultDetailState.search=this.value;renderResults()"></label><label>部門<select id="resultDepartmentFilter" onchange="resultDetailState.department=this.value;renderResults()"><option value="">全部部門</option>'+deps.map(function(dep){return '<option value="'+attr(dep)+'" '+(resultDetailState.department===dep?'selected':'')+'>'+esc(dep)+'</option>'}).join('')+'</select></label><label>顯示排序<select id="resultSortSelect" onchange="resultDetailState.sort=this.value;renderResults()"><option value="department" '+(resultDetailState.sort==='department'?'selected':'')+'>預設部門 / 員編</option><option value="oldest" '+(resultDetailState.sort==='oldest'?'selected':'')+'>最早送出</option><option value="newest" '+(resultDetailState.sort==='newest'?'selected':'')+'>最新送出</option></select></label><button class="btn success" type="button" onclick="exportFilteredResults()">Excel 匯出</button></div>';head.insertAdjacentHTML('afterend',html)}
